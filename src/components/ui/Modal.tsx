...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, label, children }) => {
    ...
    return (
        <dialog
            ...
            className="fixed inset-0 m-auto w-full max-w-4xl aspect-video bg-[var(--surface)] rounded-lg overflow-hidden p-0 border-0 focus:outline-none [&::backdrop]:bg-black/80"
        >
            {children}
            <button type="button" onClick={onClose} aria-label="Close video" className="player-close">
                ×
            </button>
        </dialog>
    );
